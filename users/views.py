...
class RecommendedFoodViewSet(mixins.ListModelMixin,
    ...
    def get_queryset(self):
        ...
        for recipe1 in service_recipe_nutrition:
            ...
            for allergent in allerta:  # for every allergent in the recipe
                for user_allergy in trans:  # loop through for each individual user provided allergent
                    if ind1 + str(
                            user_allergy) in allergent:  # check if the user_allergent matches the allergent that we are currently looping through
                        if (allerta[
                            allergent] == 'YES'):  # if the user provided allergent is actually an allergent in the recipe
                            ind = True  # set the indicator to true and break
                            break
        ...
        #     print(dieta['halal'])
        time_of_meal = 'Lunch'
        computed_cds = cds_cal.cds_cal(gender, age, lifestyle, time_of_meal, recommend1,service_menu_items_flat, hei_file)
        for recipe1 in computed_cds:
            avg_rating = None
            # Get all ratings for each recipe
            rating = ratingquery.filter(recipe_id=recipe1['recipe_id'])
            # print(n['recipe_id'])
            # Set avg_rating to aggregated avg of ratings
            if rating:
                avg_rating = rating.aggregate(Avg("rating"))['rating__avg']
            recipe1['avg_rating'] = avg_rating
        return computed_cds
